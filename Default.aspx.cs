...
public partial class Test_Default3 : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if (string.IsNullOrEmpty(Request["page"]) || !int.TryParse(Request["page"].ToString(), out pagenumber))
                pagenumber = 1;
    ...
    private void SetEndPage(string sqlstr, SqlParameter[] para)
    {
        using (SqlDataReader dr = (para == null ? SQLDB.ExecuteReader(env, "ePro", sqlstr) : SQLDB.ExecuteReader(env, "ePro", sqlstr, para)))
        {
            dr.Read();
            ucPagination.EPage = dr.GetInt32(0) % 10 == 0 ? dr.GetInt32(0) / 10 : (dr.GetInt32(0) / 10) + 1;
            dr.Close();
        }
    }
    ...
    private void Query(int pagenumber, string Number, string Branch)
    {
        ...
        if (!string.IsNullOrEmpty(Number) || !string.IsNullOrEmpty(Branch))
        {
            ...
            SetEndPage(CountSqlStr, CountPara.ToArray() as SqlParameter[]);
        }
        else
        {
            sqlstr = string.Format(@"SELECT * FROM(
            ...
            SetEndPage("SELECT count(B02002) FROM B02 where B02006='北區' and B02002 is not null", null);
        }
